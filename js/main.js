...
document.addEventListener("DOMContentLoaded", function () {
  ...
  inputs.forEach((input) => {
    ...
    input.addEventListener("blur", () => {
      setTimeout(() => {
        if (
          !document.getElementById("newItemText").matches(":focus") &&
          !document.getElementById("newItemTag").matches(":focus")
        ) {
          // Remove keyboard-open class
          document.querySelector(".app").classList.remove("keyboard-open");
  ...
  // Visual Viewport API for keyboard detection (iOS/Android)
  if ('visualViewport' in window) {
    ...
    function handleViewportResize() {
      ...
      if (keyboardHeight > 150) {
        ...
        // Also add keyboard-open class for consistency
        if (!document.querySelector('.app').classList.contains('keyboard-open')) {
          document.querySelector('.app').classList.add('keyboard-open');
        }
      } else {
    ...
    // Update initial height on window resize (orientation change)
